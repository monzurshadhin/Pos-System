...
@section('content')
    <div class="container-fluid px-4">
        <h1 class="mt-4">Purchase Update</h1>
        ...
        <form class="row g-3" action="{{route('update.order')}}" method="POST">
            @csrf
            ...
            <div class="row form-card">
                <div class="col-md-6">
                    ...
                    <select id="vendor" name="vendor_id" class="form-select" required>
                        ...
                        @foreach($vendors as $vendor)
                            <option value="{{$vendor->id}}" {{$order->vendor_id==$vendor->id?'selected':''}}>{{$vendor->vendor_name}}</option>
                        @endforeach
                    </select>
                </div>
                <div class="col-md-3">
                    ...
                    <select id="warehouse" name="warehouse_id" class="form-select" required>
                        ...
                        @foreach($wareHouses as $wareHouse)
                            <option value="{{$wareHouse->id}}" {{$order->warehouse_id==$wareHouse->id?'selected':''}}>{{$wareHouse->ware_house_name}}</option>
                        @endforeach
                    </select>
                </div>
                <div class="col-md-3">
                    ...
                    <select id="category" name="category_id" class="form-select" required>
                        ...
                        @foreach($categories as $category)
                            <option value="{{$category->id}}" {{$order->category_id==$category->id?'selected':''}}>{{$category->category_name}}</option>
                        @endforeach
                    </select>
                </div>
                <div class="col-md-6">
                ...
                <div class="col-md-3">
                    ...
                    <input type="date" name="purchase_date" class="form-control" value="{{$order->purchase_date}}" id="date" required>
                </div>
                <div class="col-md-3">
                    <label for="purchase_number" class="form-label">Purchase Number</label>
                    <input type="text" name="purchase_number" readonly value="{{$order->purchase_number}}" class="form-control" id="purchase_number">
                </div>
            </div>
            ...
            <div class="row form-card">
                ...
                <div class="col-md-3">
                    ...
                    <input type="number" name="quantity" min="0" max="5" class="form-control" id="quantity" value="{{$order->quantity}}" required>
                </div>
                <div class="col-md-2">
                    <label for="price" class="form-label" >PRICE</label>
                    @foreach($items as $item)
                        @if($item->id==$order->item_id)
                    <input type="number" name="price" value="{{$item->price}}" class="form-control" id="item_price" readonly>
                        @endif
                    @endforeach
                </div>
                <div class="col-md-1">
                    @foreach($items as $item)
                        @if($item->id==$order->item_id)
                            ...
                            <h6 class="tax" id="tax">{{$item->tax}}</h6>
                        @endif
                    @endforeach
                ...
                <div class="col-md-2">
                    ...
                    @foreach($items as $item)
                        @if($item->id==$order->item_id)
                            <input type="number" value="{{$item->discount}}" name="discount" class="form-control" readonly id="discount">
                ...
                <div class="col-md-6 mb-5">
                    ...
                    <textarea type="text" name="description" placeholder="Description" class="form-control" id="description">{{$order->description}}</textarea>
                </div>
                <hr>
            ...
            <div class="col-12 d-flex justify-content-end">
                ...
                <button type="submit" class="btn create-btn">Update Order</button>
            </div>
